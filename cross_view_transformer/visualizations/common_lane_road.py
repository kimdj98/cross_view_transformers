    ...
    def visualize(self, batch, pred=None, b_max=8, **kwargs):
        ...
        for b in range(min(batch_size, b_max)):
            if pred is not None:
                ...
                right1 = self.visualize_pred(bev[b], pred['road_bev'][b].sigmoid())
                right2 = self.visualize_pred(bev[b], pred['lane_bev'][b].sigmoid())

            else:
                right = self.visualize_bev(bev[b])

            right1 = [right1] + self.visualize_custom(batch, pred, b)
            right1 = [x for x in right1 if x is not None]

            right2 = [right2] + self.visualize_custom(batch, pred, b)
            right2 = [x for x in right2 if x is not None]

            right1 = np.hstack(right1)
            right2 = np.hstack(right2)
            ...
            if image is not None:
                ...
                if len(imgs) == 6:
                    a = np.hstack(imgs[:3])
                    b = np.hstack(imgs[3:])
                    left = resize(np.vstack((a, b)), right1) # check how this works later
                else:
                    left = np.hstack([resize(x, right1) for x in imgs])

                yield np.hstack((left, right1, right2))
            else:
                yield right
