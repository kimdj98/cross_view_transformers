...
class WppNetwork(nn.Module):
    def __init__(
        ...
        self.waypoint_predictors = nn.ModuleList([nn.LSTM(2, 1024, 2, batch_first=True) for _ in range(self.modes)])
        self.waypoint_decoder = nn.ModuleList([nn.Linear(1024, 2) for _ in range(self.modes)])
    ...
    def forward(self, batch):
        ...
        # Gather state data
        state_type = ["past_coordinate", "past_vel", "past_acc", "past_yaw"]
        ...
        # Gather prev coordinate data
        prev_coord = batch["past_coordinate"]                                                                        # (B, 5, 2)
        prev_coord = prev_coord.flip([1])                                                                       # reverse the order to make it from past to future
        ...
        y = torch.concat((y1, y2), dim=1)                                                                       # (B, 2048)
        y = self.fusion_fc(y)                                                                                   # (B, 1024)
        
        # multimodal waypoint prediction
        waypoint = torch.zeros((B, self.modes, 12, 2), device=y.device)
        h0 = y.repeat(self.num_layers, 1, 1)
        c0 = torch.zeros((self.num_layers, B, 1024), device=y.device)

        for m in range(self.modes):
            prev = prev_coord
            for i in range(12):
                output = self.waypoint_predictors[m](prev, (h0, c0))[0][:, -1, :]
                point = self.waypoint_decoder[m](output)
                waypoint[:, m, i] = point
                prev = torch.cat((prev[:, 1:], point[:, None, :]), dim=1)
