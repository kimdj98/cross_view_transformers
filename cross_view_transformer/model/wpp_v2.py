...
class WppNetwork(nn.Module):
    def __init__(
        ...
        self.conv1x1 = nn.Conv2d(6, 6*4, 1, stride=1, padding=0) 
        pos_encoding = positionalencoding2d(6*4, 200, 200)
        ...
        self.Conv = nn.Sequential(
                                nn.Conv2d(6*4, 32, 3, stride=1, padding=1),
                                nn.ReLU(),
    ...
    def forward(self, batch):
        ...
        h = torch.cat([enc_lane['bev'], enc_road['bev'].detach()], dim=1)
        lane_bev = self.lane_head(h)

        enc_curr_vehicle = self.cvt_vehicle_encoder(batch)                                                      # (b, 128, 25, 25)
        batch['image'] = batch['prev_image']
        enc_prev_vehicle = self.cvt_vehicle_encoder(batch)                                                      # (b, 128, 25, 25)   

        enc_img = self.img_encoder(batch)                                                                       # (b, n, d, fh,fw)
        enc_img = rearrange(enc_img, 'b n d h w -> b (n h w) d')                                                # (b, n*fh*fw, d)

        road_bev = enc_road['bev']
        curr_vehicle_bev = enc_curr_vehicle['bev']
        curr_vehicle_center = enc_curr_vehicle['center']
        prev_vehicle_bev = enc_prev_vehicle['bev']
        prev_vehicle_center = enc_prev_vehicle['center']

        x = torch.concat((lane_bev.detach(), 
                          road_bev.detach(), 
                          curr_vehicle_bev.detach(), 
                          curr_vehicle_center.detach(),
                          prev_vehicle_bev.detach(),
                          prev_vehicle_center.detach()
                          ),
                          dim=1)                                                 # (B, 6, 200, 200)
        
        x = self.conv1x1(x)                                                                                     # (B, 6*4, 200, 200)
        x += self.pos_encoding
