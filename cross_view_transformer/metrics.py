# min ADE metrics for multimodal trajectory prediction
import torch
...
class FDEMetric(Metric):
    ...
    def compute(self):
        result = self.min_FDE / self.total
        self.reset()
        return result
...
class MinADEMetric(Metric):
    ...
    def update(self, pred, batch):
        ...
        SE = (coord - label)**2
        DE = torch.sqrt(torch.sum(SE, dim=3))
        ADE = torch.sum(DE, dim=2) / 12
        min_ADE = torch.min(ADE, dim=1)[0]
        
        self.min_ADE += torch.sum(min_ADE)
        self.total += B
